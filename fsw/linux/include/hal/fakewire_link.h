...
#define FAKEWIRE_LINK_REGISTER(l_ident, l_options, l_rx, l_tx, l_rx_num, l_tx_num)                                    \
    ...
    TASK_REGISTER(l_ident ## _cfg, fakewire_link_configure, &l_ident, NOT_RESTARTABLE);                               \
    TASK_REGISTER(l_ident ## _rxl, fakewire_link_rx_loop, &l_ident, NOT_RESTARTABLE);                                 \
    TASK_REGISTER(l_ident ## _txl, fakewire_link_tx_loop, &l_ident, NOT_RESTARTABLE);                                 \
    fw_link_t l_ident = {                                                                                             \
        .fd_in = -1,                                                                                                  \
        .fd_out = -1,                                                                                                 \
        .rx_chart = &(l_rx),                                                                                          \
