...
macro_define(VIRTIO_CONSOLE_SCHEDULE_TRANSMIT, v_ident) {
    CLIP_SCHEDULE(symbol_join(v_ident, clip), 20)
    VIRTIO_DEVICE_QUEUE_SCHEDULE(symbol_join(v_ident, device), 3, 10) /* control.tx */
    VIRTIO_DEVICE_QUEUE_SCHEDULE(symbol_join(v_ident, device), 5, 30) /* data[1].tx */
}
...
macro_define(VIRTIO_CONSOLE_SCHEDULE_RECEIVE, v_ident) {
    ...
    VIRTIO_DEVICE_QUEUE_SCHEDULE(symbol_join(v_ident, device), 4, 30) /* data[1].rx */
}
